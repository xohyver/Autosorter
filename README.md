...
### 3.[역할] : AI 개발자
- 시스템 설계 및 구현 :
Opencv와 Yolov5를 활용하여 실시간 객체 탐지 시스템 구축, 인식된 객체의 클래스에 따라 아두이노에 신호를 전송하여 360도 서보모터를 통해 쓰레기 분류를 위한 동작 실행
- 데이터 수집 및 모델 학습 :
교내 매점 환경 특성 상 시중에 드문 물품들이 많고, 판매되는 물품의 종류가 어느정도 정해져있음. 따라서 직접 쓰레기 이미지를 수집하여 대부분의 쓰레기에 높은 정확도를 보일 수 있도록 함.
...
- 감지된 클래스 넘버에 따라 아두이노에 시리얼 통신을 통해 코드 전송, 각 코드에 따라 서보모터를 작동하여 분류를 수행함
...
### 1. 객체탐지 분류
> 카메라를 통해 쓰레기를 인식 및 분류

### 2. 시리얼 통신 활용 클래스 별 모터 동작
> Python 코드에서 시리얼 통신을 통해 아두이노의 서보모터 제어
